...
class TransRec(nn.Module):
    """
    TransRec: Transformer-based Realtime User Action Model for Sequential Recommendation at VogueGenie
    """

    def __init__(
        self,
        transrec_config: TransRecConfig,
    ):
        """
        Module Initialization
        """
        super().__init__()
        self.transrec_config = transrec_config
        self.action_emb_dim: Final[int] = self.transrec_config.action_emb_dim
        self.item_emb_dim: Final[int] = self.transrec_config.item_emb_dim
        self.concat_candidate_emb: Final[
            bool
        ] = self.transrec_config.concat_candidate_emb
        self.time_window_ms: Final[int] = self.transrec_config.time_window_ms
        self.seq_len: Final[int] = self.transrec_config.seq_len
        self.latest_n_emb: Final[int] = self.transrec_config.latest_n_emb
        self.action_vocab: Final[list] = self.transrec_config.action_vocab
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            nhead=self.transrec_config.nhead,
            dim_feedforward=self.transrec_config.dim_feedforward,
            batch_first=True,
        )
        self.transformer_encoder = nn.TransformerEncoder(
            encoder_layer, num_layers=self.transrec_config.num_layer
        )
        if self.transrec_config.concat_max_pool:
